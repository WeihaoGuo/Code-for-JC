...
for n_est in n_estimators_range:
    for min_d in min_samples_leaf:
        model = RandomForestRegressor(n_estimators=n_est, min_samples_split=3, min_samples_leaf=min_d)
        model.fit(X_train, y_train)
        y_pred = model.predict(X_test)
        r2 = r2_score(y_test, y_pred)
        results.append({'n_estimators': n_est, 'min_samples_leaf': min_d, 'R2': r2})

# heatmap
df = pd.DataFrame(results)
heatmap_data = df.pivot(columns='n_estimators', index='min_samples_leaf', values='R2')
...
plt.tight_layout()
#plt.show()
plt.savefig("heatmap_train_min_samples_leaf.png", dpi = 300);